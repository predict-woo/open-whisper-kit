# Node.js Bindings for SortFormer

Native Node.js bindings for [streaming-sortformer-ggml](https://github.com/predict-woo/streaming-sortformer-ggml), providing high-performance streaming speaker diarization in Node.js applications.

## Overview

This directory contains the Node.js addon implementation for SortFormer, NVIDIA's streaming speaker diarization model. The bindings provide a TypeScript-friendly API for real-time speaker diarization with configurable latency presets.

## Features

- **Streaming and offline diarization** with configurable latency presets
- **Up to 4 speakers** detected simultaneously
- **CoreML/ANE acceleration** on Apple Silicon (~110x real-time)
- **TypeScript support** with full type definitions
- **Zero-copy audio processing** via Float32Array
- **RTTM output** (standard diarization format) plus raw frame-level predictions
...
├── packages/
│   ├── streaming-sortformer-node/     # Main package
│   │   ├── src/
│   │   │   ├── index.ts               # Public API exports
│   │   │   ├── Sortformer.ts          # High-level wrapper class
│   │   │   ├── types.ts               # TypeScript type definitions
│   │   │   ├── presets.ts             # Latency preset configurations
...
└── src/
    ...
    ├── sortformer_model.h             # C++ model wrapper header
    └── sortformer_model.cpp           # C++ model wrapper implementation
```
...
cd streaming-sortformer-ggml/bindings/node
